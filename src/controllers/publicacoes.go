...
// BuscarPublicacao cria uma publicação
func BuscarPublicacao(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r)
	pulicacoesID, erro := strconv.ParseUint(parametros["publicacoesId"], 10, 60)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
	...
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	publicacao, erro := repositorio.BuscarPorID(pulicacoesID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
// AtualizarPublicacao atualiza uma nova publicação
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	parametros := mux.Vars(r)
	publicacaoID, erro := strconv.ParseUint(parametros["publicacaoId"], 10, 60)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
...
// DeletarPublicacao deleta uma nova publicação
