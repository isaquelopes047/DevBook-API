...
func AtualizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	// D12 - Comparar se o token que deseja alterar é o mesmo do token logado
...
// DeletarUsuario deleta o registro
...
func SeguirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possivel seguir a sí mesmo"))
		return
	}
...
// DeixarDeSeguirUsuario permite que um usuario deixei de seguir outro
func DeixarDeSeguirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possivel deixar de seguir a sí mesmo"))
		return
	}
...
// TodosOsSeguidores retorna todos os seguidores
func TodosOsSeguidores(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r)
	...
	if usuarioIdStr == "" {
		respostas.Erro(w, http.StatusBadRequest, errors.New("o ID do usuário não pode ser vazio"))
		return
	}
	...
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	seguidores, erro := repositorio.TodosOsSeguidores(usuarioID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
// BuscarSeguindo retorna todos os usuarios que um determinado usuario esta seguindo
...
// AtualizarSenha permite atualizar a senha de um usuario
